...
if(err) {
  return console.error('error fetching client from pool in user controller', err);
}
  ...
  if(err) {
    ...
    hashPassword(userPassword, function(err, hash){
      ...
      if (err){
        ...
        client.query("INSERT INTO UserLogin(username, password, email) values($1, $2, $3)", [userName, hash, userEmail], function(err, result){
          done();
          if (err) {
    ...
    callback(err, false, "Username or e-mail is unavailable");
    client.end();
  }
